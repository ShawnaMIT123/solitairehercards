    ...
    for (let suit in suits) {
      for (let value of values) {
        this.deck.push({
          suit: `${suits[suit]}`,
          value,
          image_url: `/${suits[suit]}/${value}.png`,
          hidden: true
        });
      }
    }
...
const defaultState = {
  ...
  foundation: { 1: [], 2: [], 3: [], 4: [] },
  discard: [],
  timer: { started: false, completed: null , seconds: 0}
};
  ...
  switch (action.type) {
    case UPDATE_DRAGGED_CARD:
      return Object.assign({}, state, {
        draggedCard: action.payload.draggedCard
      });
    case REMOVE_CARD_FROM_TABLEAU_COL:
      return {
        ...
        tableau: {
          ...state.tableau,
          [parseInt(action.payload.removedCard.colkey)]: state.tableau[
    ...
    case REMOVE_CARD_FROM_FOUNDATION:
      return {
        ...
        foundation: {
          ...state.foundation,
          [parseInt(action.payload.index)]: state.foundation[
    ...
    case ADD_CARD_TO_TABLEAU_COL:
      let currentTabColState = [
        ...state.tableau[action.payload.newColumn].slice(0),
        action.payload.transferredCard
      ];
    ...
    case REVEAL_CARD_IN_TABLEAU:
      return {
        ...
        tableau: {
          ...state.tableau,
          [parseInt(action.payload.newColumn)]: state.tableau[
    ...
    case ADD_CARD_TO_FOUNDATION:
      return {
        ...
        foundation: {
          ...state.foundation,
          [parseInt(action.payload.newColumn)]: [
            ...state.foundation[action.payload.newColumn].slice(0),
            action.payload.transferredCard
          ]
        }
      };
    case ADD_CARD_TO_DISCARD:
      return {
        ...state,
        discard: [...state.discard, action.payload.transferredCard]
      };
