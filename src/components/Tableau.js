             ...
             ////if card is the last card in column
               if(arr[arr.length - 1].image_url == cardUnderneath.image_url){
                 ...
                 if(draggedCard.value == cardUnderneath.value - 1  && cardUnderneathIndex%2 !== draggedCardIndex%2){
                   ...
                   }else if(flatfoundationCards.map((foundCard)=>foundCard.image_url).includes(draggedCard.image_url)){

                     let num = null
                     for(let array in this.props.foundation){
                       if(this.props.foundation[array].map((card)=>card.image_url).includes(draggedCard.image_url)){
                         num = array
                       }
...
//orginally 600
      ...
      for (let i = 0; i < 7; i++) {
        ...
        row.push(<p className="rectangle" key={i}  onDragOver={(e)=>this.onDragOver(e)} onDragStart={(e)=> this.onDragStart(e)}  onDrop={(e)=>this.onLayoutDrop(e, `${i} stack`)} style={divStyle}  />)
